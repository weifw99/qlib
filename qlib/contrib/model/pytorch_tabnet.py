...
class TabnetModel(Model):
    def __init__(
        ...
        self.device = "cuda:%d" % (GPU) if torch.cuda.is_available() and GPU >= 0 else "mps" if torch.backends.mps.is_available() else "cpu"
        self.loss = loss
        self.metric = metric
    ...
    def fit(
        ...
        for epoch_idx in range(self.n_epochs):
            ...
            if recorder is not None:
                ...
                recorder.log_metrics(step=epoch_idx, **log_m)
